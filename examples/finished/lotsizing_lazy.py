        ...
        for ell in Ts:
            ...
            if lhs < D[1, ell]:
                if checkonly:
                    ...
                    self.model.addCons(
                        quicksum([x[t] for t in L]) +
                        quicksum(D[t, ell] * y[t] for t in S) >= D[1, ell],
                        removable=True)
                cutsadded = True
        return cutsadded
...
if __name__ == "__main__":
    ...
    print("%8s%8s%8s%8s%8s%8s%12s%12s" %
          ("t", "fix", "var", "h", "dem", "y", "x", "I"))
    for t in range(1, T + 1):
        print("%8d%8d%8d%8d%8d%8.1f%12.1f%12.1f" %
              (t, f[t], c[t], h[t], d[t], model.getVal(y[t]), model.getVal(
                  x[t]), model.getVal(I[t])))
