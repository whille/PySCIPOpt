...
def mctransp(I, J, K, c, d, M):
    ...
    for i in I:
        for k in K:
            model.addCons(
                sum(x[i, j, k] for j in J if (i, j, k) in x) == d[i, k],
                "Demand(%s,%s)" % (i, k))
    # Capacity constraints
    for j in J:
        model.addCons(
            sum(x[i, j, k] for (i, j2, k) in x if j2 == j) <= M[j],
            "Capacity(%s)" % j)
    # Objective
...
def make_inst1():
    ...
    d = {
        (1, 1): 80, (1, 2): 85, (1, 3): 300, (1, 4): 6,  # {(customer,commodity):demand}}
    ...
    cost = {
        (1, 1): 4, (1, 2): 6, (1, 3): 9,  # {(customer,factory): cost}
        (2, 1): 5, (2, 2): 4, (2, 3): 7,
        (3, 1): 6, (3, 2): 3, (3, 3): 4,
        (4, 1): 8, (4, 2): 5, (4, 3): 3,
        (5, 1): 10, (5, 2): 8, (5, 3): 4
...
def make_inst2():
    ...
    cost = {
        (1, 1): 8, (1, 2): 9, (1, 3): 14,  # {(customer,factory): cost}
        (2, 1): 6, (2, 2): 12, (2, 3): 9,
...
def make_inst3():
    ...
    d = {
        (1, 1): 40, (1, 2): 30, (1, 3): 10,  # {(customer,commodity): demand}}
...
if __name__ == "__main__":
    ...
    for (i, j, k) in x:
        if model.getVal(x[i, j, k]) > EPS:
            print("sending %10s units of %3s from plant %3s to customer %3s" %
