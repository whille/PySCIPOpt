...
def make_inst():
    """prepare data for the diet model"""
...
if __name__ == "__main__":
    ...
    for b["Cal"] in [None, 3500, 3000, 2500]:
        print("\nDiet for a maximum of {0} calories".format(
            b["Cal"] if b["Cal"] is not None else "unlimited"))
        model = diet(F, N, a, b, c, d)
        model.hideOutput()  # silent mode
        model.optimize()
        x, y, z, v = model.data
        print(f"Optimal value: {model.getObjVal()}, total cost: {model.getVal(v):.2f}")
        for j in x:
            if model.getVal(y[j]) > 0:
                print(
                    "{0:30s}: {1:3.1f} dishes --> {2:4.2f} added to objective".
                    format(j, model.getVal(x[j]), model.getVal(y[j])))
